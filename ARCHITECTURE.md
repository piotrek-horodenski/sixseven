...
2. **Ranked tylko z UI serwowanym przez platformę.** Aplikacja UI zna rysowane ruchy z natury — więc w meczach rankingowych (ELO, szybki mecz) UI musi być statycznym bundlem hostowanym u nas, z CSP: `connect-src` wyłącznie API platformy + blokada WebRTC. Kanał wycieku fizycznie nie istnieje — przeglądarka gracza go blokuje. UI self-hosted = tylko mecze towarzyskie, z etykietą „UI poza platformą" w katalogu. **Filar ten dotyczy gier ZEWNĘTRZNYCH:** gry wbudowane (first-party, np. RPS) mają UI będące częścią aplikacji platformy i grają ranked natywnie — deweloperem jesteśmy my, nie ma przeciwnika, przed którym CSP miałoby bronić (ADR „Gry wbudowane vs zewnętrzne").
...
**What:** dwie klasy gier o tym samym kontrakcie. **Wbudowane:** UI w aplikacji web platformy, logika jako serwis w naszym compose, ale wołana przez TEN SAM wire contract co gry zewnętrzne (dogfooding — RPS już tak działa); ranked natywnie, bez bundla/CSP, bo filar 2 „Safe secret" broni sekretu przed *deweloperem gry*, a tu deweloperem jest platforma. **Zewnętrzne:** logika + UI w całości na infrze deva; casual z etykietą „UI poza platformą"; ranked dopiero po wgraniu UI jako zaufanego bundla na nasz hosting (CSP) — budowa tej infrastruktury odłożona do pierwszego realnego konsumenta (arrowsoccer, Etap 6).
**Watch out:** (1) Silnik nie może obrosnąć „specjalnymi ścieżkami" dla wbudowanych — różnią się wyłącznie hostingiem UI i domyślnym zaufaniem, kontrakt jest jeden. (2) Wbudowane gry nie ćwiczą ścieżki rejestracji/trustu — dogfooding samoobsługi musi robić inna gra (snajperzy w Etapie 5, arrowsoccer w Etapie 6). (3) Decyzja D2 (osobna domena rejestrowalna dla przyszłych bundli, cookies host-only) pozostaje w mocy jako zapis — realizacja przy budowie hostingu.
