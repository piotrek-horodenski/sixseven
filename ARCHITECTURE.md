...
2. **Ranked wyłącznie w grach wbudowanych.** *(zmiana 2026-07-12, sesja 2 — patrz ADR „Gry zewnętrzne = tylko towarzyskie".)* Aplikacja UI zna rysowane ruchy z natury, a UI gry zewnętrznej żyje na infrze deva — platforma nie może technicznie zagwarantować, że nie wycieka. Zamiast wymuszać hosting bundli z CSP (poprzednie brzmienie tego filaru — wycofane), gry zewnętrzne po prostu **nie grają ranked**: zawsze towarzyskie, z etykietą „UI poza platformą". Ranked biegnie tylko na grach wbudowanych (UI = część aplikacji platformy, deweloperem jesteśmy my). Gwarancja wobec gier zewnętrznych jest detekcyjna, nie techniczna: „wykrywamy i karzemy", nie „wyciek niemożliwy".
3. **Detekcja i kary.** Statystyka kontr (gracz trafiający w odpowiedzi na ruchy, których nie mógł znać, odstaje od losowości przy dużej próbie), mecze-pułapki (platforma rozgrywa mecz sterując obiema stronami przez prawdziwą aplikację gry w instrumentowanej przeglądarce i obserwuje, czy do UI wpływają dane skorelowane z „ukrytymi" ruchami), **zgłoszenia graczy** („coś było nie tak" po meczu — sygnał targetingu dla judge, nigdy dowód sam w sobie), kary od obniżenia health-score po unpublish. Po wycofaniu filaru 2 w wersji bundle+CSP warstwa 3 jest GŁÓWNĄ obroną przed nieuczciwym devem w grach zewnętrznych — świadomie przyjęte ryzyko: celowany, oszczędnie używany cheat może pozostać niewykryty (analiza motywów w ADR „Gry zewnętrzne = tylko towarzyskie").
...
| Ranked / szybki mecz | tylko gry wbudowane — gry zewnętrzne bez ranked (ADR „Gry zewnętrzne = tylko towarzyskie") |
...
- **Targeting (kogo sprawdzić):** heurystyki + opcjonalnie AI (konfigurowalne przez admina: `off` / `assist` — podpowiada adminowi / `auto` — sam alokuje budżet audytów) + ręczne zlecenia admina. Sygnały: anomalie win-rate, statystyka kontr, wzorce czasowe, grafy powiązanych kont, historia trust_events dewelopera, **zgłoszenia graczy** (report „coś było nie tak" po meczu; ważone anty-brigadingowo — masowe zgłoszenia powiązanych kont ≈ zero wagi).
...
**Watch out:** (1) Scoped tokeny to warunek bezwzględny — pełny JWT użytkownika w aplikacji gry = dev widzi czat, znajomych i cudze mecze. (2) Aplikacja gry sama rysuje timer, graczy i wynik — dane ma w `matches`, ale spójności UX nie wymusimy, jedynie podamy tokeny motywu. (3) Granica phishingu: gra nigdy nie prosi o hasło platformy — do komunikowania graczom i egzekwowania w regulaminie. (4) *(zaktualizowane 2026-07-12, sesja 2)* Hosting UI ma jeden poziom: self-hosted na domenie deva = wyłącznie mecze towarzyskie, z etykietą „UI poza platformą" (ranked tylko dla gier wbudowanych — ADR „Gry zewnętrzne = tylko towarzyskie"). Logika zawsze na infrastrukturze deva. Ewentualne osadzenie w slocie `default` jako opcja później.

### Gry wbudowane (first-party) jako osobna klasa obok gier zewnętrznych *(sesja 2026-07-12)*
**Why:** wcześniejsze dokumenty zakładały, że KAŻDA gra — także RPS — przechodzi ścieżkę zewnętrzną („RPS przechodzi na bundle" w Etapie 4). Decyzja Piotra: RPS i ewentualne 1–2 kolejne gry first-party są **stałym elementem aplikacji platformy** (minimum treści, niekonfigurowane przez nikogo), a otwarcie na deweloperów dotyczy wyłącznie gier zewnętrznych — flagowym przykładem będzie arrowsoccer na własnej domenie.
**What:** dwie klasy gier o tym samym kontrakcie. **Wbudowane:** UI w aplikacji web platformy, logika jako serwis w naszym compose, ale wołana przez TEN SAM wire contract co gry zewnętrzne (dogfooding — RPS już tak działa); ranked natywnie, bo filar 2 „Safe secret" broni sekretu przed *deweloperem gry*, a tu deweloperem jest platforma. **Zewnętrzne:** logika + UI w całości na infrze deva; wyłącznie towarzyskie z etykietą „UI poza platformą" *(zmiana 2026-07-12, sesja 2: poprzednio „ranked po wgraniu bundla z CSP" — wycofane, patrz ADR następny)*.
**Watch out:** (1) Silnik nie może obrosnąć „specjalnymi ścieżkami" dla wbudowanych — różnią się wyłącznie hostingiem UI i domyślnym zaufaniem, kontrakt jest jeden. (2) Wbudowane gry nie ćwiczą ścieżki rejestracji/trustu — dogfooding samoobsługi musi robić inna gra (snajperzy w Etapie 5, arrowsoccer w Etapie 6). (3) Status arrowsoccera jako „pierwszej zewnętrznej gry ranked" stracił podstawę — patrz otwarte pytanie w ADR następnym.

### Gry zewnętrzne = tylko towarzyskie; bundle+CSP wycofane z planu *(sesja 2026-07-12, druga)*
**Why:** UI gry zewnętrznej działa w przeglądarce gracza jako kod deva — widzi ruch w momencie powstawania (stan 1 cyklu sekretu), zanim trafi do skarbca. Technicznie domykał to filar 2 (bundle hostowany u nas + CSP), ale to ciężka infrastruktura (publish-ui, osobna domena D2, testy S3 e2e), której jedynym konsumentem byłaby przyszła zewnętrzna gra rankingowa. Decyzja Piotra: **akceptujemy, że dev może oszukiwać, dopóki tego nie wykrywamy** — gra ma się opierać na zaufaniu, które można stracić. Analiza motywów wspiera tę kalkulację: platforma nie przewiduje nagród, więc nie przyciągnie „przemysłu" cheatów; realne motywy (status, cudza monetyzacja cheatów, promocja gry, trolling) zostawiają głównie amatorów, których łapie statystyka. Koszt nazwany wprost: przy aferze obrywa marka powiernika sekretu — dlatego obietnica publiczna brzmi „wykrywamy i karzemy", nigdy „wyciek niemożliwy".
**What:** (1) Gry zewnętrzne grają WYŁĄCZNIE mecze towarzyskie, zawsze z etykietą „UI poza platformą"; ranked tylko gry wbudowane. (2) Hosting bundli + CSP + D2 + test S3 usunięte z planu (Etap 6 chudnie); zapisy D1/D2 zostają w dokumentach jako historia decyzji. (3) Obrona przed nieuczciwym devem = warstwa detekcyjna: replay-audit, test lustrzany, mecze-pułapki, statystyka kontr, zgłoszenia graczy — plus kary renomy do unpublish. (4) Podejrzenie (w tym zgłoszenia graczy) jest sygnałem targetingu, NIE publiczną miarą — publiczne pozostają renoma i sprawdzalność, oparte na dowodach (zasada „AI wybiera cele, nie ferruje wyroków" bez zmian). (5) Boty: primarnie zawodnicy do gry (dołączanie do pustego lobby), wtórnie instrument audytowy (pułapki, testowanie aplikacji zewnętrznych); bot od deva NIE jest warunkiem niczego — zachęta bez przymusu (szybszy wzrost sprawdzalności).
**Watch out:** (1) **Ranked dla gry zewnętrznej może wrócić**, jeśli znajdzie się weryfikowalna ścieżka — rozważana i na razie odrzucona: jawny silnik gry uruchamiany na naszej infrze (odrzucone: koszty obliczeń poza naszą kontrolą; wraca też problem sandboxa odrzucony wcześniej). Nie otwierać bez nowych argumentów. (2) Arrowsoccer jako gra zewnętrzna = casual; jeśli ma grać ranked, musiałby być grą wbudowaną (first-party) — DECYZJA OTWARTA, podjąć przed Etapem 6. (3) Detekcja to siatka, nie gwarancja — celowany, rzadko używany cheat może pozostać niewykryty; to ryzyko jest przyjęte świadomie i ma być widoczne w komunikacji (etykieta + regulamin). (4) Zgłoszenia graczy wymagają wag anty-brigadingowych (powiązane konta ≈ 0), inaczej stają się bronią przeciwko uczciwym devom.

### Bot-zawodnik — casual, zero ELO, subtelny sygnał *(Etap 4f, sesja 2026-07-13)*
**Why:** realizacja pomysłu Piotra „bot dołączający do gry, gdy brak przeciwnika" (pkt 5 ADR wyżej). Pełna implementacja „haka na bota" ze scaffoldu 4e — żeby lobby nie wisiało pusto, gdy nikt nie dołącza.
**What:** bot dosiada WYŁĄCZNIE do lobby (casual) z wolnym slotem, po progu czasu i tylko gdy czeka człowiek. Tożsamość: `bot_<hex>` w `matches.guestIds` — dzięki temu uczestniczy w rosterze/scoringu/reveal (iteracja `players ∪ guestIds`), ale jest **darmowo wykluczony z ELO** (`settleMatchElo` pomija mecze z gośćmi; `applyEloIfDue` iteruje tylko `players`) → zero farmingu. Bot **aktywnie losuje ruch co rundę** (crypto-entropia), NIE polega na `defaultMove` — bo `defaultMove` dla gościa bez preferencji jest deterministyczny (`hashMove(seed,pid,round)`) i przewidywalny z publicznego seedu. Złożony ruch trafia do prywatnej `moves` → replay-safe (audyt czyta zapis). UI pokazuje **subtelny, niepodrabialny marker** (ikona robota po prefiksie id, nie po nicku) — nie ukrywamy botowości przed graczem (prawdomówność). MVP: działa tylko dla RPS, przestrzeń ruchów zaszyta w platformie.
**Watch out:** (1) Ranked-backfill botem = świadomie POZA MVP (osobna decyzja o farmingu — bot w kolejce zmieniałby ELO). (2) Przestrzeń ruchów zaszyta w platformie łamie zasadę „platforma nie zna reguł gry" — dług do spłaty kontraktowym `/bot-move` (gra zwraca losowy legalny ruch) albo deklaracją ruchów w manifeście, KONIECZNE zanim bot obejmie gry zewnętrzne/arrowsoccer. (3) Bot gra niemal natychmiast (brak ludzkiego jittera) — drobny tell, łatwy do dołożenia. Pełny kontrakt: `docs/ETAP4F_BOT_CONTRACT.md`.
