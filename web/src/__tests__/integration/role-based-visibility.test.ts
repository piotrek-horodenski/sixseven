...
describe('role-based visibility', () => {
  ...
  describe('MainMenu visibility', () => {
    it('admin with all permissions sees all menu items', async () => {
      const wrapper = await mountMenu(router, [
        'view-admin', 'manage-engines', 'access-images',
        'manage-users', 'manage-roles', 'manage-settings',
      ])

      expect(wrapper.find('a[href="/engines"]').exists()).toBe(true)
      expect(wrapper.find('a[href="/images"]').exists()).toBe(true)
      expect(wrapper.find('a[href="/admin"]').exists()).toBe(true)
    })

    it('user with only access-images sees Images but not Engines or Admin', async () => {
      const wrapper = await mountMenu(router, ['access-images'])

      expect(wrapper.find('a[href="/images"]').exists()).toBe(true)
      expect(wrapper.find('a[href="/engines"]').exists()).toBe(false)
      expect(wrapper.find('a[href="/admin"]').exists()).toBe(false)
    })

    it('user with manage-engines sees Engines but not Admin', async () => {
      const wrapper = await mountMenu(router, ['manage-engines'])
  ...
  describe('route guard redirects', () => {
    ...
    it('unauthenticated user accessing /engines is redirected to /login', async () => {
      router.push('/engines')
      await router.isReady()
    ...
    it('user without manage-engines is redirected from /engines', async () => {
      loginAs(['access-images'])
    ...
    it('user with manage-engines can access /engines/list', async () => {
      loginAs(['manage-engines'])
      router.push('/engines/list')
      await router.isReady()

      expect(router.currentRoute.value.path).toBe('/engines/list')
    })
  ...
  describe('cross-permission combinations', () => {
    it('user with images + engines sees both but not admin', async () => {
      const wrapper = await mountMenu(router, ['access-images', 'manage-engines'])
    ...
    it('user can navigate between allowed sections', async () => {
      loginAs(['access-images', 'manage-engines'])

      await router.push('/images')
      expect(router.currentRoute.value.path).toBe('/images')

      await router.push('/engines/list')
      expect(router.currentRoute.value.path).toBe('/engines/list')
    })
