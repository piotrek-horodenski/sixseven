...
vi.mock('@/modules/engines/EnginesView.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/ClustersView.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EnginesSubmenu.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EngineDetailPanel.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EnginesSidebar.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/ClusterIntro.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EngineLogsView.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EngineErrorsView.vue', () => ({ default: {} }))
...
vi.mock('@/modules/engines/EnginesIntro.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/ClustersIntro.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EngineLogsIntro.vue', () => ({ default: {} }))
vi.mock('@/modules/engines/EngineErrorsIntro.vue', () => ({ default: {} }))
...
import enginesRoute from '../routes/engines.route'
...
describe('route configuration', () => {
  ...
  describe('engines routes', () => {
    it('requires manage-engines at parent level', () => {
      expect(enginesRoute.meta?.requiredPermission).toBe('manage-engines')
    })

    it('redirects to /engines/list', () => {
      expect(enginesRoute.redirect).toBe('/engines/list')
    })

    it('all children require manage-engines', () => {
      for (const child of enginesRoute.children) {
        expect(child.meta?.requiredPermission).toBe('manage-engines')
      }
    })

    it('engine-detail has aside panel', () => {
      const detail = enginesRoute.children.find((c: any) => c.name === 'engine-detail')
      expect(detail?.components?.aside).toBeDefined()
      expect(detail?.path).toBe('list/:id')
    })

    it('clusters route has sidebar', () => {
      const clusters = enginesRoute.children.find((c: any) => c.name === 'clusters')
      expect(clusters?.components?.sidebar).toBeDefined()
    })

    it('cluster-detail has intro and sidebar', () => {
      const detail = enginesRoute.children.find((c: any) => c.name === 'cluster-detail')
      expect(detail?.components?.sidebar).toBeDefined()
      expect(detail?.components?.intro).toBeDefined()
    })

    it('engines list has slide animations', () => {
      const engines = enginesRoute.children.find((c: any) => c.name === 'engines')
      expect(engines?.meta?.animation?.default?.clusters).toBe('slide-right')

      const clusters = enginesRoute.children.find((c: any) => c.name === 'clusters')
      expect(clusters?.meta?.animation?.default?.engines).toBe('slide-left')
    })

    it('has logs and errors routes', () => {
      const logs = enginesRoute.children.find((c: any) => c.name === 'engine-logs')
      const errors = enginesRoute.children.find((c: any) => c.name === 'engine-errors')
      expect(logs?.path).toBe('logs/:id')
      expect(errors?.path).toBe('errors/:id')
    })
  })
