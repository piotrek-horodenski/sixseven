...
describe('layout store', () => {
  ...
  describe('switchMenuType', () => {
    ...
    it('changes type after timeout', async () => {
      ...
      expect(store.menuType).toBe(EMenuType.vertical)
  ...
  describe('onRouteChange', () => {
    it('calls routeChange on all slots', async () => {
      const to = { matched: [], meta: {} } as any
      const from = { matched: [], meta: {} } as any

      // All slots should handle the route change without error
      await store.onRouteChange(to, from)
    })
  })
})
