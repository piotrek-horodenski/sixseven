...
describe('seed service', () => {
  ...
  it('inserts all defaults when collections are empty', async () => {
    ...
    expect(perms.map((p: any) => p.name)).toContain('play-games')
    ...
    expect(roles.find((r: any) => r.name === 'admin').useRoles).toContain('developer')
    expect(roles.find((r: any) => r.name === 'developer').useRoles).toContain('player')
    expect(roles.find((r: any) => r.name === 'player').useRoles).toContain('guest')
