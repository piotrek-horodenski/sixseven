...
| **4e** | Ranked (gry wbudowane): ELO + walkowery + kolejka szybkiego meczu (+ hak na bota) | Etap 3 (profil z 4b dostaje miejsce na ELO) | Walkower liczy ELO wg reguł; szybki mecz dobiera i rozgrywa wbudowany RPS; ranking per gra live | ✅ **live OK** (kontrakt `ETAP4_DE_CONTRACT.md`) |
| **4f** | Bot-zawodnik: dosiada do pustego lobby (casual) i losuje ruch co rundę | 4e (scaffold `bot-provider`) | Człowiek czeka w lobby RPS → po progu dosiada „Bot" (marker), gra losowo, mecz kończy się BEZ zmian ELO | 🟡 **napisane** (kontrakt `ETAP4F_BOT_CONTRACT.md`; czeka na weryfikację live Piotra) |
